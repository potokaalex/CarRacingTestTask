...
namespace Client.Code.Gameplay.GameplayCamera
{
    ...
    public class CameraController : MonoBehaviour
    {
        ...
        [Header("Position")] public bool smoothFollow; // If > 0, camera will smoothly interpolate towards the target
        ...
        [Header("Rotation")] public float rotationSensitivity = 3.5f; // The sensitivity of rotation
        ...
        [Header("Distance")] public float distance = 10.0f; // The current distance to target
        ...
        [Header("Blocking")] public LayerMask blockingLayers;
        ...
        public void UpdateTransform(float deltaTime)
        {
            ...
            if (target != null)
            {
                ...
                {
                    RaycastHit hit;
                    if (Physics.SphereCast(t - f * blockingOriginOffset, blockingRadius, f, out hit,
                            blockingOriginOffset + distanceTarget - blockingRadius, blockingLayers))
                    {
                        blockedDistance = Mathf.SmoothDamp(blockedDistance,
                            hit.distance + blockingRadius * (1f - blockedOffset) - blockingOriginOffset, ref blockedDistanceV,
                            blockingSmoothTime);
                    }
